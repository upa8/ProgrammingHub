...
public class Counting_Permutations extends MyScanner {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int qq = 0; qq < t; qq++) {
			...
			for (int i = 0; i < n; i++) {
				if ((tree[i].child.size() <= 2)
						&& !v.contains(tree[i].hash.get(-1).toString())) {
	...
	private static void dp(int i, int prev) {
		...
		if (dpv.size() == 0) {
			...
			long pow = tree[dpv.get(0)].hash.get(i).toString()
					.equals(tree[dpv.get(1)].hash.get(i).toString()) ? 1 : 2;
			tree[i].dp.put(prev,
					(((((choose(a + b, a) * dpa) % MOD) * dpb) % MOD) * pow)
							% MOD);
		}
	...
	static class Node {
		...
		Node() {
			...
			this.dp = new HashMap<Integer, Long>();
			this.hash = new HashMap<Integer, StringBuilder>();

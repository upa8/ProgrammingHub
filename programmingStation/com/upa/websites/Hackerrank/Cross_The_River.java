...
public class Cross_The_River extends MyScanner {
	...
	public static void main(String[] args) throws Exception {
		...
		Cross_The_River in = new Cross_The_River();
		PrintWriter out = new PrintWriter(new OutputStreamWriter(System.out));

		N = Integer.parseInt(in.nextToken());
		H = Integer.parseInt(in.nextToken());
		DH = Integer.parseInt(in.nextToken());
		DW = Integer.parseInt(in.nextToken());
		...
		for (int i = 0; i < N; i++) {
			...
			int z = Integer.parseInt(in.nextToken());
			ts.add(x);
			r[i] = new Rock(x, y, z);
		}
		int cnt = 1;
		...
		for (int i = 0; i < N; i++) {
			...
			if ((lo == null) || (hi == null) || (lo > hi)) {
				add(id.get(r[i].x),
						new State(r[i].y, -1 << 27, id.get(r[i].x)));
				continue;
			} else {
				lo = id.get(lo);
				hi = id.get(hi);
				...
				// out.println(max.points + r[i].points + " AT " +
				// id.get(r[i].x));
				int nextPoints = max.points == (-1 << 27) ? max.points
						: (max.points + r[i].points);
				list.add(new Update(id.get(r[i].x),
						new State(r[i].y, nextPoints, id.get(r[i].x))));
	...
	private static void add(int x, State s) {
		while ((buckets.get(x).size() > 0)
				&& (buckets.get(x).getLast().points <= s.points)) {

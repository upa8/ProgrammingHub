 ...
 * 
 * class opcpizza { private static Reader in; private static PrintWriter out;
 * 
 * public static void main(String[] args) throws IOException { in = new
 * Reader(); out = new PrintWriter(System.out, true);
 * 
 * int T = in.nextInt(); while (T-- > 0) { int N = in.nextInt(), M =
 * in.nextInt(); int [] arr = new int [N]; for (int i = 0; i < N; i++) arr [i] =
 * in.nextInt(); Arrays.sort(arr); int front = 0, back = N - 1, count = 0; while
 * (front < back) { if (arr [front] + arr [back] == M) { count++; front++;
 * back--; } else if (arr [front] + arr [back] > M) { back--; } else { front++;
 * } }
 * 
 * out.println(count); } out.close(); System.exit(0); }
 * 
 ...
 * = read(); do { ret = ret * 10 + c - '0'; } while ((c = read()) >= '0' && c <=
 * '9'); if (c == '.') while ((c = read()) >= '0' && c <= '9') ret += (c - '0')
 * / (div *= 10); if (neg) return -ret; return ret; }
 * 

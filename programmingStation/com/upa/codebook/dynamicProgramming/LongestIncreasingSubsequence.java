 ...
 * possible
 *
 * Solution : Dynamic Programming is used to solve this question. DP equation is
...
public class LongestIncreasingSubsequence {
	...
	public int longestSubsequenceRecursive(int arr[]) {
		...
		for (int i = 0; i < (arr.length - 1); i++) {
			int len = this.longestSubsequenceRecursive(arr, i + 1, arr[i]);
			if (len > maxLen) {
				maxLen = len;
			}
		}
	...
	private int longestSubsequenceRecursive(int arr[], int pos, int lastNum) {
		...
		int t1 = 0;
		if (arr[pos] > lastNum) {
			t1 = 1 + this.longestSubsequenceRecursive(arr, pos + 1, arr[pos]);
		}
		int t2 = this.longestSubsequenceRecursive(arr, pos + 1, lastNum);

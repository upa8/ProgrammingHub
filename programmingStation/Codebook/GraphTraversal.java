		...
		while ( t > 0 ) {
			...
			while ( !q.isEmpty() ) {
				...
				for ( Edge e : adj.get(v) ) {
					...
					// But we should check whether that edge is already visited or not 
					if (dist[e.dest] == (1 << 29)) {
						// If it is not visited , then make it visited 
						// Here value for visited array we are giving is 6
						// At this line , we are increment the distance of next vertex with the previous 
						// vertex 
						// If we had the value of weight , then we could have wrote following line 

...
/*
 * A = 32 ; 
 * A = A << 1 ; (Multiply A by 2^1 64 )
 * A = A >> 1 ; (Divide A by 2^1   32)
 * A = A >> 2 ; (Divide A by 2^2 which is 4 . So it will become 32/4)
 */

/*
 * To set/turn on thej-th item of the set, useS|=(1<<j).
 ...
 * 4. To check if thej-th item of the set is on (or off), use T=S&(1<<j).IfT=0,
		then thej-th item of the set is off. If T!=0, then the j-th item of the set is on.
...
/*
 * 5. To clear/turn off thej-th item of the set, use
4
S&=∼(1<<j).
S = 42 (base 10) = 101010 (base 2)
...
6. To toggle thej-th item of the set, useS
∧
=(1<<j).
 ...
 */
/*
 * 7. To get the first bit that is on (from right), useT = (S & (-S)).
...
8. To turn onallbits in a set of sizen,useS=(1<<n) - 1.
Example for n = 6
